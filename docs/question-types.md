...
Child-specific weekly practice sets generate a context `passage-question` when an example is provided, then one easy recognition card and one hard typed-answer card for each practice card; see [docs/practice-sets.md](./practice-sets.md).
...
Medium flash cards show the card and ask the student to type an answer. In language tracks such as Spanish, French, and Latin, use Medium for target-language front / English typed meaning:
...
Recommended pattern: author world-language and classical-language vocabulary ladders as separate Easy, Medium, and Hard lessons using the same deck concept. General English vocabulary can keep using Easy and Hard unless a typed recognition step is useful.

# Spanish 1 Lesson Briefs

Sources: accepted level blueprint, course map, and unit design briefs in `research/spanish-1/`.

## Unit 1: First Spanish Moves

### Lesson 1: Greetings And Polite Openings

- **Teaching goal:** Teach students to choose common greetings, farewells, and courtesy phrases for simple social contexts.
- **Student outcome:** The student can greet, say goodbye, thank someone, respond to thanks, and make a polite request with familiar chunks.
- **Key idea, model, text, representation, form, or procedure:** Greeting and politeness phrases are memorized communication moves chosen by context, not word-by-word translations.
...
- **Teaching goal:** Teach common classroom commands and help phrases students will need throughout the course.
- **Student outcome:** The student can understand and respond to phrases such as `escucha`, `lee`, `repite`, `escribe`, `mira`, `no entiendo`, and `otra vez, por favor`.
- **Key idea, model, text, representation, form, or procedure:** Classroom phrases are action cues and self-advocacy tools, not vocabulary list items.
...
- **Teaching goal:** Help students notice major Spanish sound-spelling clues in familiar words.
- **Student outcome:** The student can identify clear vowel sounds, silent `h`, `ñ`, `j`, `ll/y`, accent marks, and inverted punctuation in familiar examples.
- **Key idea, model, text, representation, form, or procedure:** Spanish spelling often gives reliable pronunciation clues, and punctuation can signal meaning before the sentence ends.
...
- **Teaching goal:** Teach students to use cognates and context to understand short Spanish exchanges while staying alert for false certainty.
- **Student outcome:** The student can identify helpful cognates, avoid overtrusting them, and build a short polite dialogue from familiar chunks.
- **Key idea, model, text, representation, form, or procedure:** Cognates are clues that help with gist when paired with context, visuals, and known phrases.
...
- **Teaching goal:** Teach numbers 0-31 through ages, dates, quantities, and short recognition tasks.
- **Student outcome:** The student can recognize, match, and produce numbers 0-31 in common Level 1 contexts.
- **Key idea, model, text, representation, form, or procedure:** Numbers are most useful when attached to meaning: age, quantity, date, time, or price.
- **Prerequisite knowledge:** Spanish sound-spelling clues and comfort with repeated oral and written retrieval.
- **Likely misconceptions:** Confusing similar forms such as `dos/doce`, `tres/trece`, `veinte/treinta`; treating number spelling as random; counting without context.
- **Evidence of mastery:** The student matches number words to numerals and uses numbers in short prompts without relying only on sequence order.
- **Recommended question types:** `match-pairs` for number-word matching; `flash-card` for retrieval; `fill-blank` for supported number choice; `text-input` after recognition.
- **Suggested intro/teaching note:** Group numbers by useful patterns, then use them immediately in ages, classroom quantities, and dates.
- **Reading-level and cognitive-load constraints:** Keep typed spelling limited and accept accent-tolerant forms where applicable; avoid long numeric strings.
...
- **Reading-level and cognitive-load constraints:** Avoid dense calendars; use familiar date numbers and short month names first.
...
- **Teaching goal:** Teach basic clock-time statements needed for schedules and routines.
- **Student outcome:** The student can interpret and produce `Es la una` and `Son las...` statements for simple times.
- **Key idea, model, text, representation, form, or procedure:** Spanish time statements distinguish one o'clock from other hours and use `y media` for half past as exposure.
- **Prerequisite knowledge:** Numbers 1-12, days and schedule context, basic singular/plural awareness.
- **Likely misconceptions:** Using `son las una`; omitting `las`; translating "at" and "is" word by word; confusing time with date.
- **Evidence of mastery:** The student matches simple clock times to Spanish statements and completes a short class-time sentence.
- **Recommended question types:** `multiple-choice` for correct time phrase; `match-pairs` for time to statement; `multi-blank-cloze` for schedule sentences; `text-input` for very short supported production.
- **Suggested intro/teaching note:** Start with `Es la una` as the special case, then show `Son las dos/tres...` as the repeating pattern.
- **Reading-level and cognitive-load constraints:** Use hour and half-hour times only; avoid minutes beyond `y media`.

## Unit 3: Classroom Objects And Descriptions

### Lesson 1: Classroom Nouns And Articles

- **Teaching goal:** Teach common classroom nouns with `el` and `la` as part of the noun phrase.
- **Student outcome:** The student can recognize classroom objects and choose the correct article for taught nouns.
- **Key idea, model, text, representation, form, or procedure:** Articles are meaningful clues that travel with nouns; students should learn `el libro`, not just `libro`.
- **Prerequisite knowledge:** Classroom survival phrases, numbers, and cognate strategy.
- **Likely misconceptions:** Treating articles as optional; assuming gender follows object meaning; choosing `el` or `la` by English translation.
- **Evidence of mastery:** The student matches objects to article-noun chunks and explains that article choice is part of the word pattern.
- **Recommended question types:** `flash-card` for object recognition; `match-pairs` for article-noun meanings; `multiple-choice` for article selection; `fill-blank` for supported article choice.
- **Suggested intro/teaching note:** Introduce each object with its article and repeat it in short classroom scenes.
- **Reading-level and cognitive-load constraints:** Teach a small set deeply; avoid lists longer than eight new nouns in one lesson.

### Lesson 2: Singular, Plural, And How Many

- **Teaching goal:** Teach students to change familiar classroom noun phrases from singular to plural and use numbers with them.
- **Student outcome:** The student can distinguish `el/la` from `los/las` and form simple plural phrases for taught nouns.
- **Key idea, model, text, representation, form, or procedure:** Number changes both the article and the noun ending in familiar Spanish noun phrases.
- **Prerequisite knowledge:** Classroom nouns with articles and numbers 0-31.
- **Likely misconceptions:** Changing the noun but not the article; adding plural endings to the wrong word; using plural with `un/una` before those are taught.
- **Evidence of mastery:** The student describes simple quantities such as `dos libros` and chooses `los` or `las` for familiar plurals.
- **Recommended question types:** `fill-blank` for article choice; `multi-blank-cloze` for quantity phrases; `error-correction` for article/noun mismatches; `match-pairs` for singular-plural pairs.
- **Suggested intro/teaching note:** Use one visible object, then two or more, and ask what changed in the Spanish phrase.
- **Reading-level and cognitive-load constraints:** Use regular plural examples first; keep irregular or spelling-change cases out.

### Lesson 3: Colors And Adjective Agreement

- **Teaching goal:** Teach basic object descriptions with color words that follow and agree with nouns.
- **Student outcome:** The student can describe familiar classroom objects with color adjectives in correct position and basic agreement.
...
- **Teaching goal:** Teach limited `ser` chunks for identifying and describing self and others.
- **Student outcome:** The student can use `soy`, `eres`, and `es` for names, identity labels, origin, and simple traits in taught contexts.
- **Key idea, model, text, representation, form, or procedure:** `Ser` helps say who someone is or what someone is like in stable identity contexts.
...
- **Teaching goal:** Teach family vocabulary and `tener` chunks for age, family, and possession.
- **Student outcome:** The student can describe a small family relationship and use `tengo/tiene` with `mi` and `tu`.
- **Key idea, model, text, representation, form, or procedure:** `Tener` can express age, having family members, and possession in familiar contexts.
...
- **Teaching goal:** Extend adjective agreement from objects to people descriptions.
- **Student outcome:** The student can describe people with familiar adjectives that agree in gender and number when applicable.
...
- **Teaching goal:** Combine identity, age, family, origin, feeling, and description language in compact profiles.
- **Student outcome:** The student can interpret and produce a short profile about self or another familiar person.
- **Key idea, model, text, representation, form, or procedure:** A novice profile is a few accurate familiar sentences, not a full paragraph with new grammar.
...
- **Teaching goal:** Teach `me gusta` as a chunk for one liked activity or singular thing.
- **Student outcome:** The student can say and interpret simple likes with `me gusta` plus an infinitive or singular noun.
- **Key idea, model, text, representation, form, or procedure:** `Me gusta` is a useful liking chunk; students should not translate it word by word.
...
- **Teaching goal:** Add plural likes and dislikes to the preference pattern.
- **Student outcome:** The student can choose between `me gusta`, `me gustan`, and `no me gusta` for taught preferences.
- **Key idea, model, text, representation, form, or procedure:** The liked thing controls `gusta/gustan`; `no` before the chunk makes a dislike.
...
- **Teaching goal:** Teach students to interpret and discuss school subjects, days, and basic class times.
- **Student outcome:** The student can read a simple schedule and say what class someone has or likes at a given time.
- **Key idea, model, text, representation, form, or procedure:** Schedules combine days, time, subjects, and opinions in a predictable text form.
- **Prerequisite knowledge:** Days, time, numbers, `me gusta`, `ser`, and school/activity vocabulary.
- **Likely misconceptions:** Reading schedules left to right without checking day/time labels; confusing `tengo clase` with age `tengo`; using English subject names when Spanish cognates are available.
- **Evidence of mastery:** The student answers schedule comprehension questions and completes a short class-time statement.
- **Recommended question types:** `passage-question` for schedule reading; `match-pairs` for subjects; `multi-blank-cloze` for class statements; `text-input` for one supported class sentence.
- **Suggested intro/teaching note:** Use a small weekly schedule and model how to find day, time, and class before answering.
- **Reading-level and cognitive-load constraints:** Use three or four classes only; avoid full school-day paragraphs.
...
- **Teaching goal:** Teach common foods and drinks with simple polite request chunks.
- **Student outcome:** The student can identify foods and drinks and request one politely using `Quiero... por favor` or recognize `Quisiera...` as a polite option.
- **Key idea, model, text, representation, form, or procedure:** Food requests combine vocabulary, article awareness, and politeness.
...
- **Teaching goal:** Connect food and drink vocabulary to meals and `tener` expressions for hunger and thirst.
- **Student outcome:** The student can say or interpret `Tengo hambre`, `Tengo sed`, and simple meal preferences.
- **Key idea, model, text, representation, form, or procedure:** Spanish uses `tener` in several familiar physical-state chunks, including age, hunger, and thirst.
...
- **Evidence of mastery:** The student chooses whether a sentence describes current location or destination and completes a place statement.
- **Recommended question types:** `multiple-choice` for meaning choice; `fill-blank` for `en/a`; `error-correction` for `estoy/voy` confusion; `passage-question` for map-note comprehension.
- **Suggested intro/teaching note:** Use two icons, one person standing in a place and one arrow going to a place, to make the meaning contrast visible.
- **Reading-level and cognitive-load constraints:** Limit places to a small community map; treat `al` as a phrase exposure when needed.
...
- **Teaching goal:** Prepare students to answer mixed novice questions across the level's domains.
...
- **Teaching goal:** Guide students to combine learned language into a compact self, family, school, likes, food, place, and routine presentation.
- **Student outcome:** The student can interpret a model capstone and produce a short supported presentation using accurate familiar chunks.
- **Key idea, model, text, representation, form, or procedure:** Level 1 mastery is clear, accurate recombination of familiar language across modes.
